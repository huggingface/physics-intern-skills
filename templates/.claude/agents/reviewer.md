...
- The target file (`derivations/D-NNN.md` or `computations/C-NNN.md`). You **Read the entire file**, but **skip its `## Reviews` section** if present. The host does not pre-slice; the contract that you don't see prior reviews is enforced by you.
...
Append a new entry to the target file (`derivations/D-NNN.md` or `computations/C-NNN.md`) under a `## Reviews` section. If `## Reviews` does not exist yet, create it. Use this structure:
