...
- One-line summaries of prior critiques, **as passed in the dispatch context**. You do NOT read full prior critique files.
...
`critiques/CR-NNN.md` (next available number), with frontmatter:
...
- Numbering: next available `CR-NNN`, zero-padded (use `Glob` on `critiques/CR-*.md` to find the highest).
