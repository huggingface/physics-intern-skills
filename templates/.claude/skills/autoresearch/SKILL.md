---
name: autoresearch
description: Run the research pipeline autonomously without human-in-the-loop gates. Can only be invoked by user.
argument-hint: "[max-iterations=N] [max-derivations=N] [start-from=survey|plan|loop|finalize]"
---
...
- `start-from=start|survey|plan|loop|finalize` — explicit resume point. Default: infer from files on disk (`{{PROBLEM_ONELINER}}` still in `CLAUDE.md` → start at `/start-research`; no `survey.md` → start at `/survey`; no `plan.md` → start at `/research-plan`; otherwise enter the dispatch loop).

## Prerequisite: workspace header

Before the loop proper, check that `/start-research` has already run: `CLAUDE.md` must not still contain the literal string `{{PROBLEM_ONELINER}}`. If it does, dispatch `/start-research` (with `from=autoresearch` in `$ARGUMENTS` so the skill returns a structured reply instead of human-facing text), integrate the one-liner into context, and continue. The user is presumed to have already filled in `problem.md` — if `/start-research` reports that placeholders are still present in `problem.md`, halt and tell the user to fill it in.
...
Everything else stays the same: the integration loop (CLAUDE.md §3) runs after every sub-agent return, sub-agents remain fresh-context, `## Flags` are dispositioned in `notes/flags.md`, commits land per integration step.
...
The log is **not** committed as a separate step. Bundle the new block into the next integration commit (the dispatch the gate authorised) by `git add notes/auto-decisions.md` alongside the integration loop's other files.
...
init:
  ...
  if CLAUDE.md still contains "{{PROBLEM_ONELINER}}":
  ...
  if entry_point == survey:
    ...
    pick next dispatch from plan.md + research_log.md open work:
      if a derivation/computation is the next step:
        ...
        if refuted: dispatch second /review; act per rule 2 of discipline reinforcements.
      ...
      if a strategy-level rewrite is warranted:
        ...
        /research-plan with feedback in $ARGUMENTS.
