...
PhysicsIntern is **not a harness.** Modern AI coding harnesses already provide tool-use loops, fresh-context sub-agent dispatch, sandboxed Python, web search, and slash-command-as-skill. We don't rebuild those. The package layers a research methodology on top: structured survey, explicit plan, fresh-context derivations and adversarial reviews, strategic critique, systematic symbolic + numerical cross-checking, citation discipline.

State lives in plain markdown files, not a typed state machine. The main agent reads and edits those files; sub-agents handle substantive work in fresh contexts. Discipline is prose-encoded (in the workspace `CLAUDE.md` and agent prompts), with periodic post-mortems via `investigate-run` to catch drift.
...
physics-intern/
├── README.md                          # this file
├── NEXT_STEP.md                       # open methodology items
...
`workspaces/` is a mix of methodology validation runs against benchmark problems (the `chNN` set) and genuine research. The repo-root `references/` directory holds the benchmark problem statements and a couple of helper scripts (`compare_entries.py`, `isolate_range.py`) used to pick and compare runs. Per-workspace `references/` (PDFs, papers cited inside a single run) live inside each workspace and are unrelated to this corpus.
...
Pre-naming the anti-patterns makes them easier to spot in audits:

- **`research_log.md` bloat** — Working Claims pile up without movement to Established or Dead Ends. Counter: invariants + periodic `/critique`.
- **Ritualistic reviews** — sub-agents return "confirmed" by default because the work looks plausible. Counter: deliberate adversarial framing; occasional independent re-reviews.
- **Single-source Established Results** — promotion without alternative evidence. Counter: hygiene rule 2, critic checks for it, audit flags it.
- **Rubber-stamped flags** — flags pile up unaddressed or are acted on without reasoning recorded. Counter: per-flag disposition in `notes/flags.md`.
- **Plan ossification** — main agent treats `plan.md` as fixed and accumulates Working Claims that don't serve the plan. Counter: `/critique`'s strategic check.
- **Citation rot** — cited artefacts have been substantially revised since the citation was written. Counter: critique spot-checks; re-review on substantial change.
- **Sub-agent scope creep** — `/derive` or `/compute` expands scope (reads other artefacts) instead of reporting back via `## Flags`. Counter: explicit prompt-level constraint.
- **Main agent becomes a passive dispatcher** — runs the pipeline without weighing flags or strategic state. Counter: checks-and-balances rule in the workspace `CLAUDE.md`.
- **Commit discipline slips** — sub-agents write artefacts but no commit lands. Counter: integration loop ties one commit to one integration step.
...
Used in active iteration. The workspace template, all 9 skills, all 7 agents, the bootstrap script, and `investigate-run` are all in place and have been exercised across the workspaces in `workspaces/`. Open methodology items are in `NEXT_STEP.md`.
