...
- **Edit `src/`, not `templates/`.** The methodology source of truth is `src/` (agents, skills, workspace doc) plus `hosts/<host>/` (host-specific glue). `templates/` and `templates-pi/` are kept only as legacy reference and are not used by the bootstrap script.
- **Render before testing changes.** After editing anything under `src/` or `hosts/`, run `bash init-physics-intern.sh --host=<host> <tmpdir>` (or call `bootstrap/render.py` directly) to verify the output before committing.
