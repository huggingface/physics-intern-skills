...
- **Host plugins.** `plugins/claude/` and `plugins/codex/` hold the authored files for the `/physics-intern:init-physics-intern` (Claude) and `$physics-intern:init-physics-intern` (Codex) plugins; `build-plugin.sh` and `build-codex-plugin.sh` assemble the publishable `physics-intern-claude-plugin` / `physics-intern-codex-plugin` repos by vendoring `commons/` + `hosts/<host>/`. Edit methodology in the source of truth and rebuild — never hand-edit a built plugin. See [`DOCUMENTATION.md`](DOCUMENTATION.md#distribution-as-a-claude-code-plugin) (Claude) and [its Codex section](DOCUMENTATION.md#distribution-as-a-codex-cli-plugin).
