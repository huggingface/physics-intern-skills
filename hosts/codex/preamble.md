...
- Sub-agent dispatch: `spawn_agent` to launch a worker (naming the role to use, e.g. `deriver`), then `wait_agent` to block until it returns. Do not use `spawn_agents_on_csv` — that is the CSV fan-out variant and is the wrong shape for one-brief-at-a-time dispatch.
