...
3. Execute the script via `bash` and capture stdout+stderr to `computations/C-NNN.out`:
   ```
   uv run python computations/C-NNN.py 2>&1 | tee computations/C-NNN.out
   ```
   (Use plain `python` if `uv` is not configured.) **Always pass an explicit timeout to `bash`** (e.g., 60–300 s for typical work, up to the tool maximum for known-heavy jobs) so a runaway computation does not hang the session. If the run times out, treat it as a signal that the approach is too expensive: shrink the problem (smaller grids, lower precision, fewer samples, simpler symbolic form) or switch paths — do not just raise the timeout blindly. The `.out` file is the durable record of what the script actually printed.
4. Write `C-NNN.md`:
