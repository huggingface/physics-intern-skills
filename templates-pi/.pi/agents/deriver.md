...
1. Read the dispatch context — typically a brief file at `derivations/.briefs/D-NNN-brief.md` whose path the main agent passes in your task description. The brief contains the target, relevant Established Results, Conventions, and specific reference file paths. **Before starting work, re-read the relevant equation, circuit, or definition directly from `problem.md`** and confirm it matches the brief. Disagreements between brief and `problem.md` are flagged loudly via `## Flags` — they often indicate a propagated error.
...
Write your structured reply at the top of your output file (the path the parent passed as `output`), so the main agent reads it directly after you finish:
...
- The number `NNN` should be the next available — use `ls derivations/D-*.md` to find the highest existing and add 1, zero-padded to 3 digits (`D-001`, `D-002`, …).
