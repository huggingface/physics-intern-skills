...
- One-line summaries of prior critiques, **as passed in the dispatch brief**. You do NOT read full prior critique files.
...
`critiques/CR-NNN.md` (next available number — use `ls critiques/CR-*.md` to find the highest), with frontmatter:
...
- Numbering: next available `CR-NNN`, zero-padded (use `ls critiques/CR-*.md` to find the highest).
